...
- **@react-native-async-storage/async-storage** - Storage engine for `redux-persist` in React Native
...
The Redux Provider is typically set up in your root layout component, for example, `app/_layout.tsx`:
...
async function handleLogout() {
  try {
    ...
    // Clear AsyncStorage if tokens are stored there
...
1. **State not persisting:**
   ...
   - Verify AsyncStorage is available (React Native) and not disabled
...
- Auth state is persisted using AsyncStorage (React Native)
