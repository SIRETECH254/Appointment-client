...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Rehydrate auth state from SecureStore/AsyncStorage and refresh the user profile in the background.
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        // This is primarily to validate token and get fresh user data on app start
        const refreshUserInBackground = async () => {
          ...
          try {
            ...
            if (userData) {
              await AsyncStorage.setItem('user', JSON.stringify(userData)); // Update stored user data
              dispatch(setAuthSuccess(userData));
            }
          } catch (refreshError: any) {
            console.log('Background token validation failed:', refreshError?.response?.status);
            // Optionally, clear auth state if token is truly invalid and not just network issue
            // For now, let interceptor handle 401 and SecureStore clear handles explicit logout
          }
        };
  ...
  // Login flow: exchange credentials for tokens and user, then persist to SecureStore/AsyncStorage.
  ...
  const updateProfile = async (
    ...
    try {
      ...
      if (updatedUser) {
        await AsyncStorage.setItem('user', JSON.stringify(updatedUser)); // Update stored user data
  ...
  const logout = async (): Promise<void> => {
    try {
      ...
      router.replace('/login'); // Changed from navigate
    }
  };
